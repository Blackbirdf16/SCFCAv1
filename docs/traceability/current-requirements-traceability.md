...
| Re-authentication for sensitive administrator actions | `backend/auth/reauth.py`, `backend/api/v1/routes/auth.py`, `backend/api/v1/routes/cases.py`, `backend/api/v1/routes/tickets.py` | `tests/test_security_hardening.py`, `tests/test_workflows.py` | Implemented | Short-lived signed `X-Reauth-Token` required for admin case creation and ticket assign/approve/reject actions. |
...
| Sensitive administrative action confirmation | Password re-authentication issues a short-lived signed token bound to the current administrator | `backend/auth/reauth.py`, `backend/api/v1/routes/auth.py`, `backend/api/v1/routes/cases.py`, `backend/api/v1/routes/tickets.py`, `frontend/src/pages/Cases.tsx`, `frontend/src/pages/Tickets.tsx` | `tests/test_security_hardening.py`, `tests/test_workflows.py` | PoC-level password confirmation only; not MFA or production privileged access management. |
...
- Features intentionally outside this PoC remain `Not implemented in PoC`: MFA, global API rate limiting, HSM/MPC custody, live blockchain execution, and production deployment claims. Re-authentication is implemented only for selected sensitive administrator actions.

## Comparison with Previous Prototype Scope

The previous prototype included some additional application-level security controls such as MFA, broader rate limiting, and asset immutability tests. The current SCFCAv2 repository focuses on a PostgreSQL-backed proof of concept, Docker Compose execution, GitLab CI/CD validation, security scanner evidence, HTML security reports, DAST/container/IaC scanning, role-aligned UI/RBAC, login throttling, and selected administrative re-authentication. Controls not implemented in SCFCAv2 are marked as deferred or out of scope rather than claimed.
...
- Current code does not show implemented MFA or global API rate limiting. Narrow in-memory failed-login throttling is implemented for the login endpoint only, and selected sensitive administrator actions require recent password re-authentication.
