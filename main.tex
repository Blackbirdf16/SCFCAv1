...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{docs//thesis-notes/uni.png}
    \label{fig:UMALOGO}
\end{figure}

\begin{titlepage}
\centering
{\Large ESCUELA TÉCNICA SUPERIOR DE INGENIERÍA INFORMÁTICA }\\[1cm]
...
The current implementation includes an auditor-only audit-chain verification endpoint. This verifier recomputes event hashes, checks continuity between the previous-hash field and the hash of the previous event, and detects broken links or tampered event content. This is stronger than simple hash lookup because it verifies the consistency of the chain rather than only checking whether a submitted hash is known.

The auditor-facing functionality also includes filtered audit review, JSON report export, HTML report export, and hash verification. The hash verification endpoint can determine whether a submitted hash corresponds to a known document hash or audit-event hash. This allows an auditor to verify integrity using safe metadata without requiring access to confidential case narratives or document contents.
...
\hline
\textbf{Phase Name} & \textbf{Phase Description} \\
\hline
\endfirsthead
...
\section{Phase-to-Implementation File Mapping}
...
\begin{longtable}{|p{1.2cm}|p{5.2cm}|p{9.4cm}|}
\hline
...
\begin{longtable}{|p{3.2cm}|p{5.2cm}|p{7.4cm}|}
\hline
