...
The SCFCA repository was organized to support both implementation and validation. It does not contain only application code; it also contains the execution environment, automated tests, security evidence, diagrams, scripts, and CI/CD configuration required to reproduce and review the proof of concept.
...
├── docs/evidence/        SBOM, security reports, and validation evidence
├── docs/diagrams/plantuml/  PlantUML sources and rendered SVG/PNG diagrams
...
This structure supports the proof-of-concept objective because the system can be executed, seeded, tested, scanned, and reviewed from the repository itself. Therefore, the repository acts not only as a code archive, but also as a validation package for the thesis.\cite{REPO1} \cite{REPO2} 

In practical terms, the proof of concept allows a reviewer to start the system with Docker Compose, seed deterministic demo data, log in as different institutional roles, observe role-based visibility differences, create and review custody workflow tickets, inspect document hash metadata, export audit reports, verify document or audit hashes, and review CI/CD security evidence generated by the repository.
...
\hline
Backend implementation & FastAPI backend with authentication, case management, ticket workflows, document handling, audit events, and hash verification & Demonstrates that the proposed architecture was translated into executable server-side logic \\
\hline
...
\hline
Persistence layer & PostgreSQL-backed users, cases, tickets, documents, assets, approvals, valuation snapshots, and audit events & Validates that access control and workflow behavior operate over persistent records rather than temporary in-memory data \\
\hline
Security controls & RBAC, signed session cookie, CSRF protection, SHA-256 document hashing, and audit hash chaining & Provides implementation evidence for the main security requirements defined in the architecture \\
\hline
...
\hline
Evidence artifacts & JSON and HTML reports, SBOM/SCA folders, security evidence folders, and retained CI artifacts & Supports auditability, traceability, and human-readable review of security evidence \\
\hline
Automated tests & Workflow tests, security hardening tests, malformed-input tests, model tests, and frontend build checks & Provides repeatable evidence that selected functional and security properties were verified \\
\hline
...
\begin{figure} [htbp]
    \centering
    \includegraphics[width=1\linewidth]{docs//diagrams//plantuml/scfca_poc_architecture.png}
...
This organization separates domain concerns such as cases, tickets, documents, audit events, and authentication into distinct backend areas, which supports maintainability and makes the implementation easier to map to the architectural models defined earlier in the thesis.
...
\begin{figure} [htbp]
    \centering
    \includegraphics[width=1\linewidth]{docs//diagrams//plantuml/backend_domain_api_structure.png}
...
This persistent dataset allows the same case, ticket, document, and audit records to be inspected from different role perspectives, making the access-control boundaries demonstrable rather than only described.
...
\begin{figure} [htbp]
    \centering
    \includegraphics[width=1\linewidth]{docs//diagrams//plantuml/scfca_docker_compose_container_model.png}
...
Each control corresponds to a specific architectural risk identified earlier in the thesis. Server-side RBAC addresses unauthorized access and privilege escalation. Assignment-based visibility addresses case data exfiltration and unauthorized case access. Ticket governance and administrator approval address bypass of custody authorization. Document hashing addresses evidence tampering. Finally, audit hash chaining addresses log manipulation and supports later verification of system activity.
...
\begin{figure} [htbp]
    \centering
    \includegraphics[width=1\linewidth]{docs//diagrams//plantuml/scfca_devsecops_pipeline_evidence.png}
    \caption{DevSecOps Pipeline PlantUML\cite{Plantuml6}}
    \label{fig:scfca-devsecops-pipeline-placeholder}
\end{figure}
...
Its value in this PoC is to show that dynamic testing can be automated and preserved as reviewable evidence, not to replace a complete penetration test.
...
After the implementation and DevSecOps phases were completed, a final reproducibility check was performed by copying the final repository into a new blank GitLab project and executing the full pipeline from scratch.
